// components/trivias/QuizGame.tsx (Updated with Premium Features)
'use client';
...
import { saveQuizResult } from '@/lib/scores';
import useWindowSize from 'react-use/lib/useWindowSize';
import QuizSummary from '@/components/trivias/QuizSummary';
import { Maximize2, Minimize2, Home, Clock, Zap, Crown } from 'lucide-react';
...
export default function QuizGame({
  ...
  const [isTimedMode, setIsTimedMode] = useState(!isPremium); // Premium users get non-timed by default
  ...
  useEffect(() => {
    ...
    /* ---------- Premium Help Functions ---------- */
  const useFiftyFifty = useCallback(() => {
    if (!isPremium || fiftyFiftyUsed || !currentQuestion || showFeedback) return;
    
    const incorrectOptions = shuffledOptions.filter(opt => 
    ...
    // Analytics
    event({action: 'fifty_fifty_used', category: 'premium', label: category});
  }, [isPremium, fiftyFiftyUsed, currentQuestion, showFeedback, shuffledOptions, category]);

  const useExtraTime = useCallback(() => {
    if (!isPremium || extraTimeUsed || !isTimedMode) return;
    ...
    // Analytics
    event({action: 'extra_time_used', category: 'premium', label: category});
  }, [isPremium, extraTimeUsed, isTimedMode, category]);

  const toggleTimedMode = useCallback(() => {
    if (!isPremium) return;
    setIsTimedMode(prev => !prev);
    setTimeLeft(timePerQuestion);
  }, [isPremium, timePerQuestion]);
  ...
  const moveToNextQuestion = useCallback(() => {
    ...
    setTimeLeft(isTimedMode ? timePerQuestion : 999); // Large number for non-timed mode
    setSelectedOption(null);
    setShowFeedback(false);
    setTimeUp(false);
    setFiftyFiftyUsed(false);
    setExtraTimeUsed(false);
  }, [timePerQuestion, isQuickfire, hasBonusQuestion, currentIndex, regularQuestions.length, showBonusQuestion, bonusQuestion, isTimedMode]);
  ...
  useEffect(() => {
    ...
    const saveQuizToDatabase = useCallback(async (finalScore: number, finalCorrectCount: number) => {
      if (!user?.id) {
      ...
      try {
        ...
        await saveQuizResult({
          ...
          time_used: timeUsed,
          is_timed_mode: isTimedMode,
          help_used: helpUsed
        }, user.id); // Pass user.id separately for security
        
        console.log('✅ Quiz result saved successfully');
      } catch (error) {
        console.error('❌ Failed to save quiz result:', error);
        // Don't throw error - we don't want to break the quiz flow
      }
    }, [user, category, subcategory, regularQuestions.length, bonusQuestion, timeUsed, isTimedMode, helpUsed]);
  ...
  const finishQuiz = useCallback(async (finalScore: number, finalCorrectCount: number) => {
    ...
    saveQuizToDatabase(finalScore, finalCorrectCount);
    
    setQuizResult(quizResult);
    setShowSummary(true);
  }, [regularQuestions.length, bonusQuestion, timeUsed, category, subcategory, isTimedMode, helpUsed, saveQuizToDatabase]);



  /* ---------- Updated handleTimeUp callback ---------- */
  const handleTimeUp = useCallback(() => {
  if (showFeedback || !gameStarted || !isTimedMode) return;
  ...
  setTimeout(() => {
    ...
    if (finished) {
      finishQuiz(score, correctCount); // Updated this line
    } else {
      moveToNextQuestion();
    }
  }, 2000);
  }, [currentQuestion, currentIndex, regularQuestions.length, showBonusQuestion, bonusQuestion, score, showFeedback, playSound, moveToNextQuestion, gameStarted, correctCount, isTimedMode, finishQuiz]);

  /* ---------- Timer ---------- */
  useEffect(() => {
    if (!gameStarted || timeLeft <= 0 || showFeedback || !isTimedMode) return;
  ...
  }, [timeLeft, showFeedback, playSound, gameStarted, handleTimeUp, isTimedMode]);

  /* ---------- Answer & next ---------- */
  const handleAnswer = useCallback(
    (option: string) => {
      if (!gameStarted || showFeedback || (timeUp && isTimedMode)) return;
      ...
      setTimeout(() => {
        ...
        if (finished) {
          event({action: 'quiz_completed', category: 'quiz', label: 'quiz'});
          finishQuiz(newScore, newCorrectCount); // Updated this line
        } else {
          moveToNextQuestion();
        }
      }, 1500);
    },
    [currentQuestion, timeLeft, score, playSound, timeUp, isQuickfire, showBonusQuestion, moveToNextQuestion, currentIndex, regularQuestions.length, bonusQuestion, gameStarted, showFeedback, correctCount, isTimedMode, finishQuiz]
  );
    ...
    <div className={containerClasses}>
      ...
      {gameStarted && currentQuestion && (
        <>
          ...
          {isPremium && (
            <div className="flex flex-wrap justify-between items-center gap-3 mb-4 p-3 bg-gradient-to-r from-purple-600 to-pink-600 rounded-xl border border-purple-500/50 shadow-lg">
              <div className="flex items-center gap-2">
              ...
              <div className="flex items-center gap-2">
                ...
                >
                  <Zap size={14} />
                  50/50
                  {fiftyFiftyUsed && <span className="text-xs">✓</span>}
                </button>
                ...
                {isTimedMode && (
                  <button
                    ...
                    disabled={extraTimeUsed || showFeedback || timeUp}
                    className={`flex items-center gap-1 px-3 py-1.5 rounded-full text-sm font-semibold transition-all ${
                      extraTimeUsed || showFeedback || timeUp
                        ? 'bg-gray-500 cursor-not-allowed text-gray-300'
                  ...
                  >
                    <Clock size={14} />
                    +15s
                    {extraTimeUsed && <span className="text-xs">✓</span>}
                  </button>
                )}
              </div>
            </div>
          ...
          <div className="grid gap-3 mb-4">
            <AnimatePresence>
              {shuffledOptions.map((opt, idx) => {
                if (hiddenOptions.includes(opt) && !showFeedback && !timeUp) {
                  return (
                    <motion.div
                      key={opt}
                      initial={{ opacity: 1 }}
                      animate={{ opacity: 0.3 }}
                      className="w-full text-left p-4 rounded-xl border border-gray-600 bg-gray-800/30 text-gray-500 cursor-not-allowed"
                    >
                ...
                return (
                  <motion.button
                    ...
                    disabled={showFeedback || (timeUp && isTimedMode)}
                    whileHover={{ scale: showFeedback || (timeUp && isTimedMode) ? 1 : 1.02 }}
                    whileTap={{ scale: 0.98 }}
                  ...
                  >
                    <div className="flex items-center justify-between">
                      ...
                      {timeUp && correct && !selected && isTimedMode && (
