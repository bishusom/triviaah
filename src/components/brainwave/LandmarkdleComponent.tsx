// src/components/brainwave/landmarkdle/LandmarkdleComponent.tsx
...
import { addLandmarkResult, type LandmarkPuzzle } from '@/lib/brainwave/landmarkdle/landmarkdle-sb';
import { checkLandmarkGuess, type LandmarkGuessResult } from '@/lib/brainwave/landmarkdle/landmarkdle-logic';
import { Home, Brain, Building, Target, Users, Search, Sparkles, Eye, EyeOff, Flame, MapPin } from 'lucide-react';
import Link from 'next/link';

interface LandmarkdleComponentProps {
  initialData: { puzzle: LandmarkPuzzle };
}

/* -------------------------------------------------------------------------- */
/*  Enhanced Helper Components                                                */
/* -------------------------------------------------------------------------- */
const EnhancedProgressiveHint = ({ attempts }: { attempts: LandmarkGuessResult[] }) => {
  ...
  const hints = [
    ...
    },
    {
      ...
      color: "bg-gradient-to-r from-cyan-500/20 to-blue-500/20 border border-cyan-500/30 text-cyan-400"
    },
    {
      icon: "💡",
    ...
    {
      icon: "⚡",
      text: "Final attempt! Use all clues and think about landmarks that fit all hints.",
        ...
        {latestAttempt.letterStatuses?.map((status, i) => (
          <div
            ...
            className={`h-1 flex-1 rounded ${
              status === 'correct' ? 'bg-gradient-to-r from-blue-400 to-cyan-500' :
...
const ValidationHints = ({ puzzleData, attempts }: { puzzleData: LandmarkPuzzle; attempts: LandmarkGuessResult[]; }) => {
  ...
  const hintItems = [
    ...
    // Hint 2: Location & Type
    attempts.length >= 2 && (
      <div key="locationType" className="flex-none w-full">
        {puzzleData.location && (
          <div className="flex items-center gap-2 mb-2">
            <span className="text-blue-400">🌍</span>
            <span className="text-white font-medium">Location:</span>
            <span className="text-cyan-400 font-bold">{puzzleData.location}</span>
          </div>
        )}
        {puzzleData.type && (
          <div className="flex items-center gap-2">
            ...
            <span className="text-cyan-400 font-bold">{puzzleData.type}</span>
          </div>
        )}
      </div>
    ),
    // Hint 3: Built Year & Architect
    attempts.length >= 3 && (
      <div key="yearArchitect" className="flex-none w-full">
        {puzzleData.builtYear && (
          <div className="flex items-center gap-2 mb-2">
            <span className="text-blue-400">📅</span>
            <span className="text-white font-medium">Built Year:</span>
            <span className="text-cyan-400 font-bold">{puzzleData.builtYear}</span>
          </div>
        )}
        {puzzleData.architect && (
          <div className="flex items-center gap-2">
            <span className="text-blue-400">👷</span>
            <span className="text-white font-medium">Architect:</span>
            <span className="text-cyan-400 font-bold">{puzzleData.architect}</span>
          </div>
        )}
      </div>
    ),
    // Hint 4: Height & Material
    attempts.length >= 4 && (
      <div key="heightMaterial" className="flex-none w-full">
        {puzzleData.height && (
          <div className="flex items-center gap-2 mb-2">
            <span className="text-blue-400">📏</span>
            <span className="text-white font-medium">Height:</span>
            <span className="text-cyan-400 font-bold">{puzzleData.height}</span>
          </div>
        )}
        {puzzleData.material && (
          <div className="flex items-center gap-2">
            ...
            <span className="text-cyan-400 font-bold">{puzzleData.material}</span>
          </div>
        )}
      </div>
    ),
...
async function fetchLandmarkImage(landmarkName: string): Promise<string | null> {
  try {
    const imageUrl = await fetchWikimediaImage(landmarkName, { entityType: 'landmark' });
...
/*  Main component                                                            */
/* -------------------------------------------------------------------------- */
export default function LandmarkdleComponent({ initialData }: LandmarkdleComponentProps) {
  const [puzzleData] = useState(initialData.puzzle);
  const [showImageModal, setShowImageModal] = useState(false);
  const [guess, setGuess] = useState('');
  const [attempts, setAttempts] = useState<LandmarkGuessResult[]>([]);
  ...
  useEffect(() => {
    const fetchImage = async () => {
      try {
        ...
        setImageError(false);
        const imageUrl = await fetchLandmarkImage(puzzleData.answer);
        if (imageUrl) {
  ...
  useEffect(() => {
    ...
    const checkGtag = setInterval(() => {
      if (typeof window !== 'undefined' && typeof window.gtag === 'function') {
        event({ action: 'landmarkdle_started', category: 'landmarkdle', label: 'landmarkdle' });
  ...
  /* --------------------------- persistence -------------------------- */
  useEffect(() => {
    const saved = localStorage.getItem(`landmarkdle-${puzzleData.id}`);
    if (saved) {
      try {
  ...
  useEffect(() => {
    if (attempts.length > 0 || gameState !== 'playing') {
      localStorage.setItem(`landmarkdle-${puzzleData.id}`, JSON.stringify({ attempts, gameState }));
    }
  ...
  const handleGuess = async () => {
    ...
    if (attempts.some(a => a.guess.toLowerCase() === normalizedGuess.toLowerCase())) {
      setErrorMessage('Already guessed this landmark');
    ...
    try {
      const result = checkLandmarkGuess(normalizedGuess, puzzleData);
      ...
      if (result.isCorrect) {
        ...
        triggerConfetti();
        playSound('win');
        await addLandmarkResult(true, newAttempts.length);
        event({ action: 'landmarkdle_win', category: 'landmarkdle', label: `attempts_${newAttempts.length}` });
      } else if (newAttempts.length >= 6) {
        setGameState('lost');
        playSound('lose');
        await addLandmarkResult(false, newAttempts.length);
        event({ action: 'landmarkdle_loss', category: 'landmarkdle', label: 'max_attempts' });
      } else {
  ...
  const generateShareMessage = () => {
    ...
    let shareText = `Landmarkdle #${puzzleNumber} ${gameState === 'won' ? attempts.length : 'X'}/6\n\n`;
  ...
  const showImage = landmarkImage && !imageLoading && !imageError;
      ...
      <div className="bg-gray-800/50 backdrop-blur-lg rounded-3xl border border-gray-700 p-5 mb-5">
        ...
        <div className="flex justify-between items-center mb-5">
          <div className="flex items-center gap-3">
            ...
            <h2 className="text-xl font-bold text-white">Guess the Mystery Landmark</h2>
          </div>
         ...
         <div className="flex flex-col md:flex-row gap-6 mb-6 items-center">
              ...
              {showImage && (
                <>
                  <Image
                    ...
                    onError={() => {
                      console.error('Image failed to load:', landmarkImage);
                      setImageError(true);
                    }}
                  />
                  {/* Block overlay */}
          ...
          <div className="flex-grow text-center">
            <div className="bg-gradient-to-r from-blue-500/10 to-cyan-500/10 border border-blue-500/30 rounded-2xl p-6">
              <h3 className="text-xl font-bold text-white mb-3">Today&apos;s Landmark</h3>
              <p className="text-gray-300 text-lg mb-4">
                Today&apos;s landmark is a <strong className="text-blue-400">{puzzleData.type}</strong>.
              </p>
        ...
        {gameState === 'won' && (
          <div className="bg-gradient-to-r from-blue-500/20 to-cyan-500/20 border border-blue-500/30 rounded-2xl p-6 mb-6 text-center">
            ...
            <p className="text-gray-300">The landmark was: <strong className="text-white">{puzzleData.answer}</strong></p>
            {attempts[attempts.length - 1]?.funFact && (
              <p className="text-cyan-400 mt-2 italic">{attempts[attempts.length - 1].funFact}</p>
            )}
          </div>
        )}
        ...
        {gameState === 'lost' && (
          <div className="bg-gradient-to-r from-red-500/20 to-pink-500/20 border border-red-500/30 rounded-2xl p-6 mb-6 text-center">
            ...
            <p className="text-red-400">The landmark was: <strong className="text-white">{puzzleData.answer}</strong></p>
              ...
              <div className="flex flex-wrap justify-center gap-2">
                {getLast7Days().map((date) => {
                  ...
                  return (
                    <Link
                      key={dateParam}
                      href={`/brainwave/landmarkdle${isToday ? '' : `?date=${dateParam}`}`}
      ...
      <div className="bg-gray-800/50 backdrop-blur-lg rounded-3xl border border-gray-700 p-5">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3 text-sm">
          <div className="flex items-start gap-2 text-gray-300">
            <span className="text-blue-400">🏛️</span>
            <span>Guess the landmark from 6 attributes</span>
          </div>
          <div className="flex items-start gap-2 text-gray-300">
            <span className="text-blue-400">🟩</span>
