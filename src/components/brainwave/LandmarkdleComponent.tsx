...
/*  Helper Components                                                         */
/* -------------------------------------------------------------------------- */
const ProgressiveHint = ({ attempts }: { attempts: LandmarkGuessResult[] }) => {
  ...
  const hintItems = [
    ...
    attempts.length >= 2 && (
      <div key="locationType" className="flex-none w-full">
        {puzzleData.location && (
          <div className="text-sm mb-2">
            🌍 Location: <strong>{puzzleData.location}</strong>
          </div>
        )}
        {puzzleData.type && (
          <div className="text-sm">
            🏛️ Type: <strong>{puzzleData.type}</strong>
          </div>
        )}
      </div>
    ),
    ...
    attempts.length >= 3 && (
      <div key="yearArchitect" className="flex-none w-full">
        {puzzleData.builtYear && (
          <div className="text-sm mb-2">
            📅 Built: <strong>{puzzleData.builtYear}</strong>
          </div>
        )}
        {puzzleData.architect && (
          <div className="text-sm">
            👷 Architect: <strong>{puzzleData.architect}</strong>
          </div>
        )}
      </div>
    ),
    ...
    attempts.length >= 4 && (
      <div key="heightMaterial" className="flex-none w-full">
        {puzzleData.height && (
          <div className="text-sm mb-2">
            📏 Height: <strong>{puzzleData.height}</strong>
          </div>
        )}
        {puzzleData.material && (
          <div className="text-sm">
            🧱 Material: <strong>{puzzleData.material}</strong>
          </div>
        )}
      </div>
    ),
    // Hint 5: Letter position & Fun Fact
    attempts.length >= 5 && (
  ...
  return (
    <div className="relative flex flex-col min-h-[calc(100vh-4rem)]">
      ...
      <div className="bg-white rounded-lg shadow-md p-4 md:p-6 mb-6 flex-grow">
        ...
        <div className="flex justify-between items-center mb-4">
          <h2 className="text-lg md:text-xl font-semibold text-gray-800">
            Guess the landmark from its attributes!
          </h2>
        ...
        {/* --------------------- PREVIOUS ATTEMPTS (UPDATED TO MATCH PLOTLE/SONGLE/CELEBRILE/CREATUREDLE) --------------------- */}
        {attempts.length > 0 && (
          <div className="mb-6">
        ...
        {(gameState === 'won' || gameState === 'lost') && (
          <div className="flex flex-col items-center mt-4">
            ...
            {shareMessage && <div className="mt-2 text-blue-600">{shareMessage}</div>}
      ...
      <div className="bg-gray-100 rounded-lg p-4 mt-6">
        <h3 className="font-bold mb-2">How to Play Landmarkdle:</h3>
        <ul className="list-disc list-inside space-y-1 text-sm">
          <li>Guess the landmark by entering its name</li>
          ...
          <li>Additional hints about the landmark unlock with each attempt</li>
          <li>The landmark image becomes clearer with each guess</li>
          <li>You have 6 attempts to guess the landmark</li>
        </ul>
      </div>
    </div>
  );
