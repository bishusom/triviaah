...
/* -------------------------------------------------------------------------- */
/*  (All helper components – ProgressiveHint, ValidationHints, PosterBlock)   */
/* -------------------------------------------------------------------------- */
  ...
  const hintItems = [
    ...
    attempts.length >= 2 && (
      <div key="habitatDiet" className="flex-none w-full">
        {puzzleData.habitat && (
          <div className="text-sm mb-2">
            🌍 Habitat: <strong>{puzzleData.habitat}</strong>
          </div>
        )}
        {puzzleData.diet && (
          <div className="text-sm">
            🍽️ Diet: <strong>{puzzleData.diet}</strong>
          </div>
        )}
      </div>
    ),
    ...
    attempts.length >= 3 && (
      <div key="sizeActivity" className="flex-none w-full">
        {puzzleData.size && (
          <div className="text-sm mb-2">
            📏 Size: <strong>{puzzleData.size}</strong>
          </div>
        )}
        {puzzleData.activity && (
          <div className="text-sm">
            🌅 Activity: <strong>{puzzleData.activity}</strong>
          </div>
        )}
      </div>
    ),
    ...
    attempts.length >= 4 && (
      <div key="bodyContinents" className="flex-none w-full">
        {puzzleData.bodyCovering && (
          <div className="text-sm mb-2">
            🛡️ Body Covering: <strong>{puzzleData.bodyCovering}</strong>
          </div>
        )}
        {puzzleData.continent && (
          <div className="text-sm">
            🗺️ Continent: <strong>{puzzleData.continent}</strong>
          </div>
        )}
      </div>
    ),
  ...
  return (
    <div className="relative flex flex-col min-h-[calc(100vh-4rem)]">
      ...
      <div className="bg-white rounded-lg shadow-md p-4 md:p-6 mb-6 flex-grow">
        ...
        {/* --------------------- PREVIOUS ATTEMPTS (UPDATED TO MATCH PLOTLE/SONGLE/CELEBRILE) --------------------- */}
        {attempts.length > 0 && (
          <div className="mb-6">
