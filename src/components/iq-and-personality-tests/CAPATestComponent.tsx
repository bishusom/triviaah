...
const CAPATestComponent: React.FC<CAPATestComponentProps> = ({ 
  ...
  const downloadAsPDF = async () => {
    ...
    try {
      ...
      pdfDoc.setKeywords(['CAPA', 'Cognitive', 'Assessment', 'Elite Trivias']);
      pdfDoc.setProducer('elitetrivias.com');
      pdfDoc.setCreator('Elite Trivias Brainwave Assessment Tool');
      ...
      // Add footer text
      const fontSize = 8;
      const footerText = `Generated by elitetrivias.com • ${new Date().toLocaleDateString()} • Profile: ${result.profileType}`;
