...
export async function getDailyFact(customDate?: Date): Promise<FactResult | null>  {
  ...
  console.log(`Fetching daily fact for date: ${dateString}`);
  ...
  if (dailyRecords && dailyRecords.length > 0) {
    ...
    console.log(`Found daily record with fact_id: ${dailyRecord.fact_id}`);
    ...
    if (!factError && factRecords && factRecords.length > 0) {
      ...
      console.log(`Successfully fetched scheduled fact`);
      return {
        fact: {
          id: dailyRecord.id,
      ...
      console.error('Fact not found with ID:', dailyRecord.fact_id, factError);
      console.log('Scheduled fact ID is invalid, falling back to random fact');
    }
  } else {
    console.log(`No daily records found for date: ${dateString}`);
  }

  // STEP 3: Fallback to random_trivia_facts view - returns array!
  console.log('Falling back to random_trivia_facts view');
  ...
  if (!randomFactError && randomFacts && randomFacts.length > 0) {
    console.log('Using random fact from random_trivia_facts view');
    return {
      fact: {
        id: `random-${Date.now()}`,
        date: dateString,
  ...
  console.log('Random view failed, fetching all facts from trivia_facts');
  ...
  if (!allFactsError && allFacts && allFacts.length > 0) {
    ...
    console.log(`Using fallback fact from ${allFacts.length} total facts`);
    return {
      fact: {
